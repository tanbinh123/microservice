...
export class InterfacesDataPermissionAssignmentComponent implements OnInit {
  ...
  interfacesUserRoleDataPermissionResponse_user:InterfacesUserRoleDataPermissionResponse = new InterfacesUserRoleDataPermissionResponse();
  interfacesUserRoleDataPermissionResponse_role:InterfacesUserRoleDataPermissionResponse = new InterfacesUserRoleDataPermissionResponse();

  resultPage_user:ResultPage = new ResultPage();//分页结果初始化;
  resultPage_role:ResultPage = new ResultPage();//分页结果初始化;
  url:String;
  id:string;
  entity:string;
  ...
  public userDataPermission(interfacesUserRoleDataPermissionResponse:InterfacesUserRoleDataPermissionResponse):void {
    this.httpService.requestJsonData(ApiConstant.SYS_INTERFACES_USER_ROLE_DATA_PERMISSION,JSON.stringify(interfacesUserRoleDataPermissionResponse)).subscribe(
      {
        next:(result:any) => {
          //console.log(result);
          if(result.code==200){
  ...
  public dataPermissionAssignment():void {
    ...
    this.httpService.requestJsonData(ApiConstant.SYS_INTERFACES_DATA_PERMISSION_ASSIGNMENT,JSON.stringify(userRolePermissionRequest),new Map([['interfacesId',this.id]])).subscribe(
      {
        next:(result:any) => {
          if(result.code==200){
            alert('数据权限分配成功');
