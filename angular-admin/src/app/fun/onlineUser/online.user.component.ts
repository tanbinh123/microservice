...
export class OnlineUserComponent implements OnInit {
  ...
  constructor(public httpService:HttpService,
    ...
    this.onlineUserListZone = authService.canShow(ApiConstant.SYS_ONLINE_USER_LIST);
    this.onlineUserOfflineZone = authService.canShow(ApiConstant.SYS_ONLINE_USER_OFFLINE);
  }
  ...
  onlineUserListRequest:OnlineUserListRequest = new OnlineUserListRequest();//在线用户列表搜索条件
  ...
  public onlineUserListFunction(onlineUserListRequest:OnlineUserListRequest):void {
    this.httpService.requestJsonData(ApiConstant.SYS_ONLINE_USER_LIST,JSON.stringify(onlineUserListRequest)).subscribe(
      {
        next:(result:any) => {
          //console.log(result);
          if(result.code==200){
  ...
  public onlineUserOffline(eachData):void {
    this.httpService.requestJsonData(ApiConstant.SYS_ONLINE_USER_OFFLINE,null,new Map([['key',eachData.userId+',1,1']])).subscribe(
      {
        next:(result:any) => {
          //console.log(result);
          if(result.code==200){
