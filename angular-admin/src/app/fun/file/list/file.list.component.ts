...
export class FileListComponent implements OnInit {
  ...
  constructor(public router:Router,
    ...
    this.fileListZone = authService.canShow(ApiConstant.SYS_FILE_LIST);
    this.fileContentListZone = authService.canShow(ApiConstant.SYS_FILE_CONTENT_LIST);
    this.fileUploadZone = authService.canShow(ApiConstant.SYS_FILE_UPLOAD_FILE);
    this.fileDownloadZone = authService.canShow(ApiConstant.SYS_FILE_DOWNLOAD_FILE);
    //this.fileDetailZone = authService.canShow(ApiConstant.SYS_FILE_DETAIL);
    this.fileDeleteZone = authService.canShow(ApiConstant.SYS_FILE_DELETE);
  }
  ...
  fileDownloadSrc:string = ApiConstant.SYS_FILE_DOWNLOAD_FILE;
  fileListRequest:FileListRequest = new FileListRequest();//文件列表搜索条件
  resultPage:ResultPage = new ResultPage();//分页结果初始化

  fileContentBackFlag:boolean = false;
  folderPath:string = null;
  fileContentListRequest:FileContentListRequest = new FileContentListRequest();//文件列表搜索条件
  resultPage2:ResultPage = new ResultPage();//分页结果初始化
  ...
  public fileRootPathFunction():void {
    this.httpService.requestJsonData(ApiConstant.SYS_FILE_ROOT_PATH).subscribe(
      {
        next:(result:any) => {
          //console.log(result);
          if(result.code==200){
  ...
  public fileListFunction(fileListRequest:FileListRequest):void {
    this.httpService.requestJsonData(ApiConstant.SYS_FILE_LIST,JSON.stringify(fileListRequest)).subscribe(
      {
        next:(result:any) => {
          //console.log(result);
          if(result.code==200){
  ...
  public fileContentListFunction(fileContentListRequest:FileContentListRequest):void {
    this.httpService.requestJsonData(ApiConstant.SYS_FILE_CONTENT_LIST,JSON.stringify(fileContentListRequest)).subscribe(
      {
        next:(result:any) => {
          //console.log(result);
          if(result.code==200){
  ...
  public fileUpload(folderPath):void {
    if(this.fileFormData!=null){
      ...
      this.httpClient.post(ApiConstant.SYS_FILE_UPLOAD_FILE,this.fileFormData,options).subscribe(
        {
          next:(result:any) => {
            //console.log(result);
            if(result.code==200){
  ...
  public fileDwonloadFunction(id:string,originFileName:string):void{
    //window.open(ApiConstant.SYS_FILE_DOWNLOAD_FILE+'/'+id+'?token='+this.sessionService.getHeadToken().token);
    this.httpClient.get(ApiConstant.SYS_FILE_DOWNLOAD_FILE+'/'+id+'?token='+this.sessionService.getHeadToken().token,{responseType:'blob'}).subscribe(blob =>{
  ...
  public fileDeleteFunction(id:string):void{
    this.httpService.requestJsonData(ApiConstant.SYS_FILE_DELETE+'/'+id).subscribe(
      {
        next:(result:any) => {
          //console.log(result);
          if(result.code==200){
