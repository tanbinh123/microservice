...
public class MathUtil {
	...
	//简单计算两个数的最小公倍数
	public static int getLeastCommonMultipleForEasy(int one,int another){
		int min = 0;
		int leastCommonMultiple = 1;//若无最小公倍数，则默认最小公倍数是1
		if(one>another){
			min = another;
		}else{
			min = one;
		...
		if(one>=another){
			if(one==another){
				...
				if(one%another==0){
					...
					return getLeastCommonMultipleForEasy(another,one%another);
				}
			}
		}else{
			//两数交换
			...
			another=one^another;
			one=one^another;
			return getLeastCommonMultipleForEasy(one,another);
		}
		*/
	...
	//简单计算两个数的最大公约数
	public static int getGreatestCommonDivisorForEasy(int one,int another){
		int result = getLeastCommonMultipleForEasy(one,another);
		int greatestCommonDivisor = result*(one/result)*(another/result);
    ...
    //BigDecimal大数值比较（-1为小于；0为等于；1为大于）。注：equals比较值和精度，compareTo比较值；数字类型转BigDecimal应该用BigDecimal.valueOf
